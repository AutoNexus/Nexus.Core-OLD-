...
namespace Nexus.Selenium.Elements.Actions
{
    ...
    public class JsActions
    {
        ...
        private Browser Browser => NexusServices.Browser;

        private IElementActionRetrier ActionRetrier => NexusServices.Get<IElementActionRetrier>();
        ...
        public T FindElementInShadowRoot<T>(By locator, string name, ElementSupplier<T> supplier = null, ElementState state = ElementState.Displayed)
            where T : IElement
        {
            var shadowRootRelativeFinder = new RelativeElementFinder(Logger, NexusServices.ConditionalWait, ExpandShadowRoot);
            var shadowRootFactory = new ElementFactory(NexusServices.ConditionalWait, shadowRootRelativeFinder, NexusServices.Get<ILocalizationManager>());
